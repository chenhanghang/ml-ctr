...
const std::string FmModel::_model_spliter = " ";
...
void FmModel::train(const std::string & train_path, const std::string & test_path) {
...
//测试
void FmModel::test(const std::string & test_path) {
    std::ifstream infile;
...
void FmModel::train_SGD(const std::string & train_path, const std::string & test_path) {
    //开始迭代
    for (int i=0; i<this->_epoch; i++) {
        ...
        while (getline(infile,s))  {
            ...
            for (std::vector<int>::size_type i=0; i<x.size(); i++) {
                ...
                for (int f=0; f < this->K; f++) {
                    //更新vif
                    vi[f] -= this->_alpha*(y*(prob-1)*(x[i].second*sum_vf[f] - vi[f]*std::pow(x[i].second, 2)) );
                    vi[f] = this->_intersect(vi[f]);
                }
                //std::cout<<"&&&"<<x[i].first<<std::endl;
                //更新wi
                this->_model[x[i].first].w -= this->_alpha*y*(prob-1)*x[i].second;
...
void FmModel::train_BGD(const std::string & train_path, const std::string & test_path) {
    //开始迭代
    for (int i=0; i< this->_epoch; i++) {
        std::cout<<"epoch "<<i<<std::endl;
        std::ifstream infile;
        ...
        while (!infile.eof()) {
            ...
            for (auto sample : batch_samples) {
                ...
                for (std::vector<int>::size_type i=0; i<x.size(); i++) {
                    ...
                    for (int f=0; f < this->K; f++) {
                        //更新vif
                        vi[f] -= this->_alpha*(y*(prob-1)*(x[i].second*sum_vf[f] - vi[f]*std::pow(x[i].second, 2)) )/this->_batch;
                        vi[f] = this->_intersect(vi[f]);
                    }
                    //std::cout<<"&&&"<<x[i].first<<std::endl;
                    //更新wi
                    this->_model[x[i].first].w -= this->_alpha*y*(prob-1)*x[i].second/this->_batch;
                    this->_model[x[i].first].w = this->_intersect(this->_model[x[i].first].w);
                }    
                //更新b即w0
...
void FmModel::train_FTRL(const std::string & train_path, const std::string & test_path) {
