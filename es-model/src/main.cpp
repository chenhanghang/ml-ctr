...
void process(std::unique_ptr<::ml::ctr::CtrModel> &ctr_model) {
    ...
    if(FLAGS_mode == "train") {
        ...
        ctr_model->set_threshold(FLAGS_threshold);
...
int main(int argc, char* argv[]) {
    ...
    if(FLAGS_algorithm == "lr") {
        ctr_model.reset(new ::ml::ctr::LrModel(FLAGS_model_path, FLAGS_mode));
    } else if(FLAGS_algorithm == "fm"){
        ctr_model.reset(new ::ml::ctr::FmModel(FLAGS_model_path, FLAGS_mode));
    } else if(FLAGS_algorithm == "ffm") {
        ctr_model.reset(new ::ml::ctr::FfmModel(FLAGS_model_path, FLAGS_mode));
    }
    process(ctr_model);
    return 0;
}
